...
I tried to work with the enum `pg_query::NodeRef`. This uses a nice pattern I hadn't seen before,
here's an excerpt:
...
Each of the variants of `NodeRef` is a reference to a protobuf struct
...
and you avoid having to match on the enum.
...
`Vec<Node>` in its `.table_elt` attribute, instead of `ColumnDef`, so we must either match and
